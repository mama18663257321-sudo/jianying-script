...
var 更新配置 = {
    ...
    备用直链: "https://raw.fastgit.org/mama18663257321-sudo/jianying-script/main/%E5%89%AA%E6%98%A0%E8%87%AA%E5%8A%A8%E5%8C%96%E8%84%9A%E6%9C%AC.js",
    当前版本: "1.0.2"
};
...
function 检查更新环境() {
    ...
    // 检查存储权限
    var 测试路径 = "/sdcard/Download/test_permission.txt";
    try {
...
function 重启脚本(脚本路径) {
    try {
        ...
        // 方法1: 直接执行新脚本
        engines.execScriptFile(脚本路径);
        
        // 等待新脚本启动
        sleep(3000);
        
        // 方法2: 如果方法1失败，显示提示信息
        显示日志("📋 如果脚本没有自动重启，请手动运行: " + 脚本路径);
    ...
    threads.start(function() {
        try {
            ...
            try {
                ...
                // 尝试备用链接
                try {
                    响应 = http.get(更新配置.备用直链, {
                        timeout: 15000
                    });
                } catch (e2) {
                    显示日志("❌ 备用链接也失败: " + e2.toString());
                    return;
                }
            }
            
            if (响应 && 响应.statusCode === 200) {
                ...
                if (网络脚本.substring(0, 100) !== 当前脚本.substring(0, 100)) {
                    ...
                    if (是否更新) {
                        // 使用更安全的文件路径
                        var 文件路径 = "/sdcard/Download/剪映自动化脚本.js";
                        
                        显示日志("📥 下载新版本到: " + 文件路径);
                        
                        // 确保目录存在
                        if (!files.exists("/sdcard/Download/")) {
                            files.createWithDirs("/sdcard/Download/");
                        }
                        ...
                        if (files.exists(文件路径)) {
                            var 文件大小 = files.size(文件路径);
                            显示日志("✅ 更新文件写入成功，大小: " + 文件大小 + " 字节");
                            显示日志("🔄 准备重启脚本...");
                            sleep(2000);
                    ...
                    显示日志("✅ 当前已是最新版本 " + 更新配置.当前版本);
                }
            } else {
                显示日志("❌ 检查更新失败: HTTP " + (响应 ? 响应.statusCode : "无响应"));
            }
        } catch (e) {
            // 详细的错误信息
            var 错误信息 = e.toString();
            ...
            if (错误信息.includes("Permission denied")) {
                显示日志("💡 提示: 请授予存储权限");
            } else if (错误信息.includes("ENOENT") || 错误信息.includes("No such file")) {
                显示日志("💡 提示: 文件路径不存在");
            } else if (错误信息.includes("timeout")) {
                显示日志("💡 提示: 网络连接超时");
            } else if (错误信息.includes("Unable to resolve host")) {
                显示日志("💡 提示: 无法解析域名，请检查网络连接");
            } else if (错误信息.includes("Unexpected end of stream")) {
                显示日志("💡 提示: 网络连接被中断");
            }
        }
    });
...
function 强制停止单个应用(包名, 应用名) {
    ...
    // 查找停止按钮
    显示日志("🔍 正在查找停止按钮");
    var 停止按钮 = text("结束运行").findOne(2000) || 
                  text("强行停止").findOne(2000) ||
                  text("强制停止").findOne(2000) ||
                  textMatches(/.*结束运行.*|.*强行停止.*|.*强制停止.*/).findOne(2000);
    
    if(停止按钮) {
        显示日志("✅ 找到停止按钮: " + 停止按钮.text());
        ...
        if(停止按钮.click()) {
            ...
            显示日志("❌ 停止按钮点击失败，尝试坐标点击");
            var bounds = 停止按钮.bounds();
    ...
    var 确认按钮 = null;
    for(var i = 0; i < 3; i++) {
        确认按钮 = text("确定").findOne(1000) || 
                  text("确认").findOne(1000) ||
                  text("强行停止").findOne(1000) ||
                  textMatches(/.*确定.*|.*确认.*/).findOne(1000);
        if(确认按钮) break;
        sleep(1000);
    }
    
    if(确认按钮) {
        显示日志("✅ 找到确认按钮: " + 确认按钮.text());
        
        // 点击确认按钮 - 使用try catch防止异常
        try {
            if(确认按钮.click()) {
                显示日志("🖱️ 确认按钮点击成功");
            } else {
                显示日志("❌ 确认按钮点击失败，尝试坐标点击");
                var bounds = 确认按钮.bounds();
                click(bounds.centerX(), bounds.centerY());
            }
            sleep(2000);
        } catch (e) {
...
function 执行第十步() {
    ...
    // 读取上次选择的索引
    var 存储文件 = "/sdcard/Download/视频选择索引.txt";
    var 当前索引 = 0;
